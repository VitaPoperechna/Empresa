<?php include 'basico.php';?>
    <main class="container d-flex me-md-auto">
        <div class="col-lg-6 px-3">
            <h1 class="display-4 fst-italic">¡Bienvenido al mundo del seguro de automóvil fiable!</h1> 
            <p class="lead my-3"><a href="/empresa/contacto.php" class="text-body-emphasis fw-bold">Tramitación rápida</a>, condiciones transparentes y asistencia 24/7. Proteja su vehículo hoy mismo.</p>
        </div>
        <div class="col-lg-6 px-3">
            <img src="img/public.svg" class="img-responsive" alt="public" width="600">
        </div>
    </main>
    <?php include 'footer.php';?>
</body>
</html>